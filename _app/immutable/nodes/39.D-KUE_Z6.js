import{s as on,n as pn}from"../chunks/scheduler.B0TnBjPs.js";import{S as cn,i as ln,e as s,s as l,c as vt,H as Ut,a as o,g as p,b as i,d as R,f as xt,h as n,n as Ht,o as c,j as a,p as u,m as wt,t as Ct,k as Tt,l as yt}from"../chunks/index.CHfggXAQ.js";import{V as _t}from"../chunks/ViewApp.6tllL5xD.js";function un(nn){let h,Mt="<code>Updater</code>",Q,T,St="On this page you find the documentation for the GUI Component <code>Updater</code>.",Z,x,Pt='<div class="remark-container__title">Note!</div><p><code>Updater</code> is not an ordinary GUI component. It can be created the same way as a GUI component, but it is not part of the GUI itself. Instead, it is used to dynamically create a child component, and it is the child component that is part of the GUI. So you can&#39;t call configuration methods like <code>.size()</code> or <code>.backgroundColor()</code> on the <code>Updater</code> component. Do that on the child component it creates instead.</p>',K,y,It="Introduction",tt,_,Lt='By default, the GUI can only be changed by taking the user to another page (including to the same page), such as when the user clicks on a <a href="/documentation/button/" title="Open the documentation for Button.">Button</a>. But, with the <code>Updater</code> component, you can update a part of the GUI on the current page whenever you want, without taking the user to another page.',nt,f,U,bt="Example",B,Gt='Example showing what the <code>Updater</code> component can do (in this case updating a <a href="/documentation/text/" title="Open the documentation for Text.">Text</a> component to show the current time every second).',H,et,M,Vt="<code>childCreator()</code> - Specifying the GUI",at,S,Et="Use the configuration method <code>childCreator()</code> to tell the <code>Updater</code> component how it should create the part of the GUI it is responsible for. Pass it a function that creates and returns a GUI component that represent that part of the GUI.",st,P,jt="The function you pass to <code>childCreator()</code> will be called immediately when the user comes to the page, but it can also be called at later points in time to create a new version of the GUI for that part.",ot,r,I,$t="Example",D,At="In this example, the method passed to <code>childCreator()</code> is only called when the user directly comes to to the page, so the usefulness of the <code>Updater</code> component is not shown, but it demonstrated how one can use <code>childCreator()</code>.",L,m,Ot="Open in Online Editor",F,pt,en=`<code class="language-js"><span class="token keyword">class</span> <span class="token class-name">StartPage</span> <span class="token keyword">extends</span> <span class="token class-name">Page</span><span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`,ct,b,Rt="<code>interval()</code> - Setting the interval",lt,G,Bt="Use the configuration method <code>interval()</code> to tell the <code>Updater</code> component how often it should update the part of the GUI it is responsible for. Pass it the time as the number of milliseconds between the updates.",it,d,V,Dt="Example",N,Ft="In this example, the method passed to <code>childCreator()</code> will be called when the user comes to the page, and then also every second after that.",E,g,Nt="Open in Online Editor",z,ut,an=`<code class="language-js"><span class="token keyword">class</span> <span class="token class-name">StartPage</span> <span class="token keyword">extends</span> <span class="token class-name">Page</span><span class="token punctuation">&#123;</span>
...
<span class="token punctuation">&#125;</span></code>`,rt,j,zt="<code>name()</code> - Manually triggering updates",dt,$,qt="Use the configuration method <code>name()</code> to give your <code>Updater</code> component instance a name. You can then call the function <code>runUpdater()</code> and pass it the same name whenever you want the <code>Updater</code> to update its GUI.",kt,w,Xt='<div class="remark-container__title">Warning!</div><p>Triggering manual updates of the GUI like this tends to make your code hard to read and hard to understand how it works. If you can achieve the same result by loading the page anew, that is usually better to do than triggering your own manual updates.</p>',ht,C,Yt='<div class="remark-container__title">Note!</div><p>A click on a <a href="/documentation/button/" title="Open the documentation for Button.">Button</a> does by default take the user to the next page. If you want a part of the GUI to be updated on the current page instead, you must call the configuration method <code>.stay()</code> on the <code>Button</code> to not take the user to the next page.</p>',ft,k,A,Wt="Example",q,Jt="In this example, the displayed time is only updated when you click on the <code>Button</code>.",O,v,Qt="Open in Online Editor",X,mt,sn=`<code class="language-js"><span class="token keyword">class</span> <span class="token class-name">StartPage</span> <span class="token keyword">extends</span> <span class="token class-name">Page</span><span class="token punctuation">&#123;</span>
